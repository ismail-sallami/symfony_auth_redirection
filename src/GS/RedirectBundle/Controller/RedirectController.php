<?php

namespace GS\RedirectBundle\Controller;

use Symfony\Component\HttpFoundation\Request;
use Symfony\Bundle\FrameworkBundle\Controller\Controller;

use GS\RedirectBundle\Entity\Redirect;
use GS\RedirectBundle\Form\RedirectType;

/**
 * Redirect controller.
 *
 */
class RedirectController extends Controller
{

    /**
     * Lists all Redirect entities.
     *
     */
    public function indexAction()
    {
        $em = $this->getDoctrine()->getManager();

        $entities = $em->getRepository('GSRedirectBundle:Redirect')->findAll();

        return $this->render('GSRedirectBundle:Redirect:index.html.twig', array(
            'entities' => $entities,
        ));
    }
    /**
     * Creates a new Redirect entity.
     *
     */
    public function createAction(Request $request)
    {
        $entity = new Redirect();
        $form = $this->createCreateForm($entity);
        $form->handleRequest($request);

        if ($form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $em->persist($entity);
            $em->flush();

            return $this->redirect($this->generateUrl('redirect_show', array('id' => $entity->getId())));
        }

        return $this->render('GSRedirectBundle:Redirect:new.html.twig', array(
            'entity' => $entity,
            'form'   => $form->createView(),
        ));
    }

    /**
    * Creates a form to create a Redirect entity.
    *
    * @param Redirect $entity The entity
    *
    * @return \Symfony\Component\Form\Form The form
    */
    private function createCreateForm(Redirect $entity)
    {
        $form = $this->createForm(new RedirectType(), $entity, array(
            'action' => $this->generateUrl('redirect_create'),
            'method' => 'POST',
        ));

        $form->add('submit', 'submit', array('label' => 'Create'));

        return $form;
    }

    /**
     * Displays a form to create a new Redirect entity.
     *
     */
    public function newAction()
    {
        $entity = new Redirect();
        $form   = $this->createCreateForm($entity);

        return $this->render('GSRedirectBundle:Redirect:new.html.twig', array(
            'entity' => $entity,
            'form'   => $form->createView(),
        ));
    }

    /**
     * Finds and displays a Redirect entity.
     *
     */
    public function showAction($id)
    {
        $em = $this->getDoctrine()->getManager();

        $entity = $em->getRepository('GSRedirectBundle:Redirect')->find($id);

        if (!$entity) {
            throw $this->createNotFoundException('Unable to find Redirect entity.');
        }

        $deleteForm = $this->createDeleteForm($id);

        return $this->render('GSRedirectBundle:Redirect:show.html.twig', array(
            'entity'      => $entity,
            'delete_form' => $deleteForm->createView(),        ));
    }

    /**
     * Displays a form to edit an existing Redirect entity.
     *
     */
    public function editAction($id)
    {
        $em = $this->getDoctrine()->getManager();

        $entity = $em->getRepository('GSRedirectBundle:Redirect')->find($id);

        if (!$entity) {
            throw $this->createNotFoundException('Unable to find Redirect entity.');
        }

        $editForm = $this->createEditForm($entity);
        $deleteForm = $this->createDeleteForm($id);

        return $this->render('GSRedirectBundle:Redirect:edit.html.twig', array(
            'entity'      => $entity,
            'edit_form'   => $editForm->createView(),
            'delete_form' => $deleteForm->createView(),
        ));
    }

    /**
    * Creates a form to edit a Redirect entity.
    *
    * @param Redirect $entity The entity
    *
    * @return \Symfony\Component\Form\Form The form
    */
    private function createEditForm(Redirect $entity)
    {
        $form = $this->createForm(new RedirectType(), $entity, array(
            'action' => $this->generateUrl('redirect_update', array('id' => $entity->getId())),
            'method' => 'PUT',
        ));

        $form->add('submit', 'submit', array('label' => 'Update'));

        return $form;
    }
    /**
     * Edits an existing Redirect entity.
     *
     */
    public function updateAction(Request $request, $id)
    {
        $em = $this->getDoctrine()->getManager();

        $entity = $em->getRepository('GSRedirectBundle:Redirect')->find($id);

        if (!$entity) {
            throw $this->createNotFoundException('Unable to find Redirect entity.');
        }

        $deleteForm = $this->createDeleteForm($id);
        $editForm = $this->createEditForm($entity);
        $editForm->handleRequest($request);

        if ($editForm->isValid()) {
            $em->flush();

            return $this->redirect($this->generateUrl('redirect_edit', array('id' => $id)));
        }

        return $this->render('GSRedirectBundle:Redirect:edit.html.twig', array(
            'entity'      => $entity,
            'edit_form'   => $editForm->createView(),
            'delete_form' => $deleteForm->createView(),
        ));
    }
    /**
     * Deletes a Redirect entity.
     *
     */
    public function deleteAction(Request $request, $id)
    {
        $form = $this->createDeleteForm($id);
        $form->handleRequest($request);

        if ($form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $entity = $em->getRepository('GSRedirectBundle:Redirect')->find($id);

            if (!$entity) {
                throw $this->createNotFoundException('Unable to find Redirect entity.');
            }

            $em->remove($entity);
            $em->flush();
        }

        return $this->redirect($this->generateUrl('redirect'));
    }

    /**
     * Creates a form to delete a Redirect entity by id.
     *
     * @param mixed $id The entity id
     *
     * @return \Symfony\Component\Form\Form The form
     */
    private function createDeleteForm($id)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('redirect_delete', array('id' => $id)))
            ->setMethod('DELETE')
            ->add('submit', 'submit', array('label' => 'Delete'))
            ->getForm()
        ;
    }

    public function redirectAction($source)
    {        
        $em = $this->getDoctrine()->getManager();
        $repository = $em->getRepository('GSRedirectBundle:Redirect');

        $redirect = $repository->findOneBySourceURL($source);

        $destination_url = $redirect->getDestinationURL();
        
        return $this->redirect($destination_url);
    }
}
